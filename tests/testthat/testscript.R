...
test_that("unsparse generic", {expect_true(isGeneric("unsparse"))})
test_that("standardize generic", {expect_true(isGeneric("standardize"))})
...
test_that("unsparse formals", {
  expect_true(length(formals(unsparse)) >= 1L)
})

test_that("standardize formals", {
  expect_true(length(formals(standardize)) >= 1L)
})
...
test_that("check returned class for mult", {
  expect_s4_class({
    x <- as(c(0, 1, 0, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
    sparse_mult(x, y)
  }, "sparse_numeric")
})
...
test_that("sparse_add", {
    result <- as(c(1, 0, 0, 1, 6), "sparse_numeric")
    expect_equal({
        x <- as(c(0, -1, 0, 1, 2), "sparse_numeric")
        y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
        sparse_add(x, y)
...
test_that("sparse_mult", {
  result <- as(c(0, 1, 0, 0, 8), "sparse_numeric")
  expect_equal({
    x <- as(c(0, 1, 0, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
    sparse_mult(x, y)
...
test_that("sparse_sub", {
  result <- as(c(-1, 0, 0, 1, -2), "sparse_numeric")
  expect_equal({
    x <- as(c(0, 1, 0, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
    sparse_sub(x, y)
...
test_that("unsparse", {
  result <- c(1, 4, 2, 3)
  expect_equal({
    x <- as(c(0, 1, 0, 0, 0, 4, 2, 0, 0, 3, 0), "sparse_numeric")
    unsparse(x)
  }, result)
})
...
test_that("standardize", {
  x <- c(0, 1, 0, 1, 2)
  result <- (x - base::mean(x)) / sd(x)
  expect_equal({
    y <- as(x, "sparse_numeric")
    standardize(y)
  }, result)
})
